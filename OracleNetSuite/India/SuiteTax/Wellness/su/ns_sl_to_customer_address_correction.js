    ...
    function(record, runtime, search, serverWidget, encode, file,common, https,url, _) {
        ...
        function onRequest(context) {
			...
			var CategoryField = Form.addField({
				 ...
				 CategoryField.addSelectOption({value: 1,text: 'All Franchisees'});
				 CategoryField.addSelectOption({value: 2,text: 'All Inter Group Entities'});
				 CategoryField.addSelectOption({value: 3,text: 'Others'});
				 CategoryField.addSelectOption({value: 4,text: 'XMS Customers'});
...
search.createColumn({
	  ...
	  search.createColumn({name: "custentity_psg_ei_entity_edoc_standard",label: "edoc_standard"}),
	  search.createColumn({name: "custentity_psg_ei_auto_select_temp_sm",label: "select_temp_sm"}),
	  ...
	  search.createColumn({ name: "formulatext", formula: "SUBSTR({taxregistration.taxregistrationnumber}, 1,2)",label: "gstin_two_digit"})
				...
				var InvObj = common.searchAllRecord('customer',null,FiltersArray,ColumnsArray);
			var data_invoice = common.pushSearchResultIntoArray(InvObj);
			
			var InvObj_tax_field = common.searchAllRecord('customer',null,FiltersArray,ColumnsArray_tax_field);
			var data_invoice_tax_field = common.pushSearchResultIntoArray(InvObj_tax_field);
				...
				xmlString += '<Worksheet ss:Name="Customer Address Report">';
				...
				'<Cell><Data ss:Type="String"><strong>  TEMPLATE AND SENDING METHOD AUTO-SELECTION</strong></Data></Cell>' +
				'<Cell><Data ss:Type="String"><strong> E-DOCUMENT PACKAGE</strong></Data></Cell>' +
				...
				'</Row>';
						...
						for (var k = 0; k < data_invoice.length; k++) { 
							...
							var gstin = '-';
							 ...
							 var internalid = data_invoice[k].internalid;
							   var address = data_invoice[k].address;
							     var select_temp_sm = data_invoice[k].select_temp_sm;
							...
							if(obj_vvr){
							 ...
							 log.debug('end_date obj_vvr', JSON.stringify(obj_vvr));
							  if(address == ""){address = '-'; }
							 // var category = data_invoice[k].category;
							  var category =  InvObj[k].getText({ name: 'category'});;
							  var edoc_standard =  InvObj[k].getText({ name: 'custentity_psg_ei_entity_edoc_standard'});;
...
'<Cell><Data ss:Type="String">' + select_temp_sm + '</Data></Cell>' +					
				'<Cell><Data ss:Type="String">' + edoc_standard + '</Data></Cell>' +				  
			...
			var fileName = 'Customer_address_report.xls';
		...
		function setSublistReorderPoint(sublist, Form, request, response,custpage_category) {
		 ...
		 sublist.addField({ id: 'custpage_addressinternalid', type: serverWidget.FieldType.TEXT, label: 'Address ID' });
			...
			sublist.addField({ id: 'custpage_select_temp_sm', type: serverWidget.FieldType.TEXT, label: 'TEMPLATE AND SENDING METHOD AUTO-SELECTION' });
			sublist.addField({ id: 'custpage_is_edoc_standard', type: serverWidget.FieldType.TEXT, label: 'E-DOCUMENT PACKAGE' });
      ...
      search.createColumn({ name: "taxregistrationnumber", join: "taxRegistration", label: "GSTIN"}),
	   ...
	   search.createColumn({ name: "formulatext", formula: "SUBSTR({taxregistration.taxregistrationnumber}, 1,2)",label: "gstin_two_digit"})
        ...
        sublist.setSublistValue({  id: 'custpage_reg_type', line: lineNum, value: regist_type });
         if(edoc_standard != ""){
		sublist.setSublistValue({  id: 'custpage_is_edoc_standard', line: lineNum, value: edoc_standard });
		 }
		  if(select_temp_sm != ""){
		sublist.setSublistValue({  id: 'custpage_select_temp_sm', line: lineNum, value: select_temp_sm });
		  }
