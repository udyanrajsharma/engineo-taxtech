...
var inv_data =	GetInvoiceToCancel(); 
 ...
 var Form = serverWidget.createForm({
    ...
    displayType : serverWidget.FieldDisplayType.HIDDEN
		...
		var ewbNo = objRecord.getValue({fieldId: 'custbody_in_eway_bill_no'});
		log.debug('ewbNo ewbNo', JSON.stringify(ewbNo));
		if (ewbNo) {
			
			var can_rsn = ["","DUPLICATE", "DATA_ENTRY_MISTAKE", "ORDER_CANCELLED", "OTHERS"];
			
			log.debug('ewbNo cancel_reason', JSON.stringify(can_rsn[cancel_reason]));
			
			var ewbCancelPayload = {
				"ewbNo": ewbNo,
				"cancelRsnCode":  can_rsn[cancel_reason],
				"cancelRmrk" : cancel_remark
				};
			
			log.debug('ewbNo cancel_reason', JSON.stringify(ewbCancelPayload));
			var ewbnexus = objRecord.getValue({fieldId: 'nexus'});
			var ewbsubsidiary = objRecord.getValue({fieldId: 'subsidiary'});
			
			var ewb_Seller_Details = GetSellerDetails(ewbsubsidiary, ewbnexus);	 
			var ewb_Api_Token = GetApiToken(1);
			var ewb_token_val = ewb_Api_Token[0].token;			
			var ewb_gstin_val = ewb_Seller_Details[0].taxregistrationnumber;	
				
			 var ewbCancelUrl = "https://api.clear.in/einv/v2/eInvoice/ewaybill/cancel";
			 var ewbCancelHeaders = new Array();
			 ewbCancelHeaders['X-Cleartax-Auth-Token'] = ewb_token_val;
			 ewbCancelHeaders['Content-Type'] = 'application/json';
			 ewbCancelHeaders['Accept'] = 'application/json';
			 ewbCancelHeaders['gstin'] = ewb_gstin_val;
			 
			 var ewbCancelResponse = https.post({url: ewbCancelUrl, body : JSON.stringify(ewbCancelPayload), headers: ewbCancelHeaders});
			 
			 if(ewbCancelResponse.code == 200) {
				 objRecord.setValue({ fieldId: 'custbody_e_way_bill_status', value: '4'});
			 }
			 else{
				 log.error('E-Way Bill Cancellation Failed', JSON.stringify(ewbCancelResponse))
				 log.error('E-Way Bill Cancellation Failed body', JSON.stringify(ewbCancelResponse.body))
			 }
...
var is_success =	body_val[0].govt_response.Success;				
		...
		function GetSellerDetails(subsidiary,nexus) 
	{
	  ...
	  var subsidiary_data = common.searchAllRecord('subsidiary',null,filters,columns); 
		 ...
		 log.debug(' Seller_Details', JSON.stringify(Seller_Details));
