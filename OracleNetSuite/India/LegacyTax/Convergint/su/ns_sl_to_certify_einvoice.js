 ...
 */, function (
  ...
  function onRequest(context) {
    ...
    var psg_ei_content = JSON.parse(
      objRecord.getValue({ fieldId: "custbody_generated_edoc" })
    );
    ...
    if (is_b2c) {
      var url_new = "https://api.clear.in/einv/v1/b2c/generate-qr-code";
      ...
      if (is_success == null) {
        ...
        objRecord.setValue({
          fieldId: "custbody_b2c_transaction_id",
          value: body_val.transaction_id,
        });
        objRecord.setValue({ fieldId: "custbody_qr_code", value: ei_qrcode });
        objRecord.setValue({ fieldId: "custbody_ei_status", value: 4 });
        objRecord.setValue({ fieldId: "custbody_edoc_status", value: "Y" });
      } else {
        var success_msg = "Failled";
        objRecord.setValue({ fieldId: "custbody_ei_status", value: 8 });
      }
      objRecord.setValue({
        fieldId: "custbody_certified_edoc",
        value: file_id,
      });
      ...
      if (is_success == "Y") {
        ...
        objRecord.setValue({ fieldId: "custbody_ei_status", value: 4 });
        objRecord.setValue({
          fieldId: "custbody_einvoice_error",
          value: error_msg,
        });
        var currentDateTime = new Date();
        objRecord.setValue({
          fieldId: "custbody_e_doc_generation_time",
