    ...
    function(record, runtime, search, serverWidget, encode, file,common, https,url, _,moment) {
        ...
        function onRequest(context) {
			...
			rec_type = 'CreditMemo';
			 ...
			 // is SEZ
			 var custbody187 = objRecord.getValue({fieldId: 'custbody_v_invoice_client_sez'});
			  ...
			  var state_code_buyer = shipstate;
				...
				var billstate = ship_Details[0].billstate;
				 ...
				 state_code_buyer_detail = State_Details[billstate_index].state_code;
					  ...
					  log.debug('billstate billstate billstate billstate', JSON.stringify(billstate));
		  ...
		  var edocContent = [
    {
        "transaction": {
			...
			"ContrDtls": [
          {
            ...
            "BuyerDtls": {
                "Gstin": client_gst,
                "LglNm": Buyers_Details[0].altname,
...
var fileName = rec_type+'_'+tranid+'_edoc.json';
				var File = file.create({
					...
					var edoc_id = File.save();
					 var today = new Date();
					 if(rec_type == 'Invoice'){	
var otherId = record.submitFields({type: record.Type.INVOICE,id: invoice_id,values: {'custbody_generated_edoc': JSON.stringify(edocContent),'custbody_ei_status' : 2,'custbody_generated_edoc_file' : edoc_id,'custbody_einvoice_gen_date' : today}});
otherId = 200;
}else{
	
	var otherId = record.submitFields({type: 'creditmemo',id: invoice_id,values: {'custbody_generated_edoc': JSON.stringify(edocContent),'custbody_ei_status' : 2,'custbody_generated_edoc_file' : edoc_id,'custbody_einvoice_gen_date' : today}});
otherId = 200;
			...
			var Ship_Details = common.pushSearchResultIntoArray(ship_data);
			  for (var p = 0; p < Ship_Details.length; p++) {
				 ...
				 if(addressinternalid == billstate){
					 var gstin = Ship_Details[p].gstin; 
					 if(gstin != ""){
						  break;
					 }
				 }
...
return state_Details;		
	...
	function GetShippingDetails(invoice_id) 
	{
	  ...
	  search.createColumn({name: "custrecord_india_gstin", join: "billingAddress",label: "bill_gstin" }),
