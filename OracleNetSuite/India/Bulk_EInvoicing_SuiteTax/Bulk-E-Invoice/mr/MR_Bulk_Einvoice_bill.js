...
function(record, search, runtime, error, file, task,common,moment,url,https,email,render) {
	...
	var invoice_data = scriptObj.getParameter('custscript_invoice_data_map_bill');
	var gen_edoc_folder = scriptObj.getParameter('custscript_gen_edoc_folder_bill');
	
	var inv_pdf_folder = scriptObj.getParameter('custscript_inv_pdf_folder_bill');
	
	var certify_edoc_folder = scriptObj.getParameter('custscript_certify_edoc_folder_bill');
	var email_sender = scriptObj.getParameter('custscript_edoc_email_sender_bill');
	var bulk_tax_url = scriptObj.getParameter('custscript_bulk_tax_url_bill');
		...
		try {
		 ...
		 var objRecord = record.load({
			...
			rec_type = 'CreditMemo';
			 ...
			 var tranid = objRecord.getValue({fieldId: 'transactionnumber'});
			 ...
			 log.debug('taxtotal25 taxtotal25', JSON.stringify(taxtotal25));
			 log.debug('taxtotal27 taxtotal27', JSON.stringify(taxtotal27));
			 var total = objRecord.getValue({fieldId: 'total'});
			
	var	subtotal =	(parseFloat(total)-(parseFloat(taxtotal25)+parseFloat(taxtotal26)+parseFloat(taxtotal27) ))
			 
			// var subtotal = objRecord.getValue({fieldId: 'subtotal'});
			 log.debug('subtotal subtotal', JSON.stringify(subtotal));
		...
		var Buyers_Details = GetBuyersDetails(entity);	 
		 log.debug('Buyers_Details Buyers_Details', JSON.stringify(Buyers_Details));   
		if(location_val != ""){
		...
		var Item_Details = GetItemDetails_Creditmemo(objRecord,invoice_id);	 
			...
			//	var dispatchStcd = ship_Details[0].entitytaxregnum.substring(0, 2);
		  ...
		  log.debug('Item values values  ', JSON.stringify(values));   
		  ...
		  var edocContent = [
    {
        "transaction": {
            ...
            "BuyerDtls": {
                ...
                "Pos": state_code_buyer_detail[0],
                "Addr1": billaddress1,
                "Addr2": billaddress2,
                "Loc": billcity,
                "Pin": billzip,
                "Stcd": state_code_buyer_detail[0],
                "Ph": null,
	...
	log.debug('edocContent edocContent', JSON.stringify(err));
}
...
var fileName = rec_type+'_'+tranid+'_edoc.json';
				var File = file.create({
					...
					contents: JSON.stringify(edocContent, null, "\t")
					...
					var edoc_id = File.save();
					 ...
					 try {
		if(rec_type == 'Invoice'){		 
var otherId = record.submitFields({type: 'vendorbill',id: invoice_id,values: {'custbody_psg_ei_content': JSON.stringify(edocContent),'custbody_psg_ei_status' : 19,'custbody_psg_ei_generated_bulk_edoc' : edoc_id,'custbody_bulk_einvoice_gen_date' : today}});
		}else{
		var otherId = record.submitFields({type: 'vendorcredit',id: invoice_id,values: {'custbody_psg_ei_content': JSON.stringify(edocContent),'custbody_psg_ei_status' : 19,'custbody_psg_ei_generated_bulk_edoc' : edoc_id,'custbody_bulk_einvoice_gen_date' : today}});	
			
		}
		}
catch(err) {
	
	log.debug('err err', JSON.stringify(err));
}
...
return Token_Details;		
	...
	function GetShippingDetails(invoice_id) 
	{
		 ...
		 var filters=[    ["type","anyof","VendBill","VendCred"], 
      "AND", 
      ["mainline","is","T"], 
      "AND", 
	  ...
	  var columns=[
	  search.createColumn({name: "billaddress1", label: "shipaddress1"}),
      search.createColumn({name: "billaddress2", label: "shipaddress2"}),
      search.createColumn({name: "billcity", label: "shipcity"}),
      search.createColumn({name: "billstate", label: "shipstate"}),
      search.createColumn({name: "billzip", label: "shipzip"}),
...
return Seller_Details;		
	...
	function GetBuyersDetails(entity) 
	{
	  ...
	  var customer_data = common.searchAllRecord('vendor',null,filters,columns); 
...
return Dispatch_Details;		
	...
	{ 
      ...
      log.debug(' CertifyEdocInvoice start', JSON.stringify(invoice_id));
		...
		log.debug('gstin_val gstin_val', JSON.stringify(gstin_val));
