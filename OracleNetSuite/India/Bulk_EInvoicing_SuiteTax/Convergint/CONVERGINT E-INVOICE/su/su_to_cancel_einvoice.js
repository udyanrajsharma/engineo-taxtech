 ...
 * @NModuleScope Public
 */
define(['N/record', 'N/runtime', 'N/search', 'N/ui/serverWidget', 'N/encode', 'N/file', "../lib/common_2.0", "N/https", "N/url",'N/task',"../lib/moment.js"],
    /**
     * @param {record} record
     * @param {runtime} runtime
     * @param {search} search
     */
    function(record, runtime, search, serverWidget, encode, file,common, https,url,task,moment) {
...
var currentISTDateTime = new Date(currentUTCDateTime.getTime() + ISTOffsetMilliseconds);
				 ...
				 var is_data_display = false;
				  ...
				  var dateTime = new Date(ack_date_einvoice);
								...
								var minutes = dateTime.getMinutes(); 
									...
									is_data_display = true;	
									}else if (day_curr > day && hours == hours_curr && minutes >minutes_curr){
										is_data_display = true;	
...
return Token_Details;		
    ...
    ["custbody_einvoice_gen_date","onorafter","yesterday"]  ];
	   ...
	   var ColumnsArray = [
				search.createColumn({name: "tranid",sort: search.Sort.DESC, label: "tranid" }),				 
				 ...
				 search.createColumn({name: "custbody_e_doc_generation_time", label: "ack_date_einvoice"}),
				 search.createColumn({name: "custbody_einvoice_gen_date", label: "ack_date"})
       ];
	...
	function setSublistReorderPoint(sublist, Form, request, response,start_date,end_date) {
      ...
      search.createColumn({name: "custbody_einvoice_gen_date", label: "einvoice_gen_date"}),
      search.createColumn({name: "custbody_e_doc_generation_time", label: "ack_date_einvoice"}),
