...
class IRISeinv:
    ...
    def einvoice_v(from_date, to_date, created_by, request_id):
        ...
        for row in Header_Eibv_data:
            ...
            einvDataModel.initiateEinvoiceFilingForInvoice(response_payload[0],response_payload[1],response_payload[3],response_payload[4], created_by, request_id)
            response_einv = einvDataModel.fileEinvData(response_payload[0],response_payload[2],response_login[0],response_login[1])
            einvDataModel.finishEinvFilingForInvoice(response_einv[0],response_einv[1],response_payload[1])
