...
class gstDataModel:
    ...
    def createGstr1Paylod(row):
        try:
            ...
            servicelogger_info.info(f"Payload creted for GSTR1 report for Invoice No: {row[0]}")
            print("Payload creted for GSTR1 report for Invoice")
    ...
    def createGstr2Paylod(row):
        try:
            ...
            print("\nPayload Created")
            servicelogger_info.info(f"Payload creted for GSTR2 report for Invoice No: {row[0]}")
            return payload, invoice_id, gstIn, invoice_date, return_period
        except Exception as e:
            servicelogger_error.exception(f"Exception occured in payload creation for GSTR1")
